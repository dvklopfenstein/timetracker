...
def run_paid(cfgproj, uname, amount, paid_at=None, **kwargs):
    ...
    if fcsv is None:
        ...
        return None  # {'fcsv':fcsv, 'csvline':None}

    now = get_now() if 'now' not in kwargs else kwargs['now']
    paidtime = now if paid_at is None else get_dtz(paid_at, now, kwargs.get('defaultdt'))
    if paidtime is None:
    ...
    csvfields = get_ntcsv(
        ...
        kwargs.get('activity'),
        kwargs.get('tags'))
    csvline = wr_csvline(fcsv, paidtime, delta, csvfields)
