...
def test_cfg_reinit(project='baking', trksubdir='.chef'):
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        cfg = run_reinit(ntexpdirs.cfglocfilename,
                         dircsv=None,
                         project=project,
                         dirhome=ntf.dirhome)
        findhome(tmpdir)
        doc_loc = _chk_cfgs(cfg, exp_docloc_glbcfg=None)
        assert exists(dflt_glbcfg)
        assert doc_loc.global_config_filename is None
        ...
        cfg = run_reinit(ntexpdirs.cfglocfilename,
                         ...
                         project=project,
                         dirhome=ntf.dirhome,
                         fcfg_global=ntf.gfname_a)
        ##cfg.reinit(project=project,
        ##           dircsv=None,
        ##           fcfg_global=ntf.gfname_a,
        ##           dirhome=ntf.dirhome)
        findhome(tmpdir)
        doc_loc = _chk_cfgs(cfg, exp_docloc_glbcfg=ntf.gfname_a)
        assert doc_loc.global_config_filename == ntf.gfname_a, doc_loc.global_config_filename
        ...
        ##           dirhome=ntf.dirhome)
        findhome(tmpdir)
        doc_loc = _chk_cfgs(cfg, exp_docloc_glbcfg=ntf.gfname_b)
        assert doc_loc.global_config_filename == ntf.gfname_b, doc_loc.global_config_filename
        #run.chk_init_proj(cfg, ntdirs, trksubdir, ntf.gfname_a)
...
def _chk_cfgs(cfg, exp_docloc_glbcfg):
    ...
    doc_loc = get_docproj(cfg.cfg_loc.filename)
    assert doc_loc is not None
    assert doc_loc.project is not None
    # Check that project is added to list in CfgGlobal
    projects = cfg.cfg_glb.get_projects()
    assert projects
    proj_glb, proj_cfg = projects[-1]
    assert proj_glb == doc_loc.project
    assert proj_cfg == cfg.cfg_loc.filename
    assert doc_loc.global_config_filename == exp_docloc_glbcfg, \
        f'docloc["global_config"]["filename"] -- '\
        f'EXP({exp_docloc_glbcfg}) != '\
        f'ACT({doc_loc.global_config_filename}) '
    if exp_docloc_glbcfg:
        assert exists(exp_docloc_glbcfg), exp_docloc_glbcfg
    return doc_loc
